...
///
/// Weapons pass `null` when the repair material is the tier's own `ToolMaterial` material — that keeps the
/// vanilla tag (`stone_tool_materials` for T0, iron ingot for T1, diamond for T2, netherite ingot for T3,
/// gold ingot for GOLDEN, `wooden_tool_materials` for WOODEN).
///
/// Contents are emitted by `PaladinsDataGenerator.ItemTagGenerator`.
public class PaladinItemTags {
    ...
    private static TagKey<Item> repairs(String material, List<Identifier> required, List<Identifier> optional) {
        var tag = TagKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(PaladinsMod.ID, "repairs_" + material));
